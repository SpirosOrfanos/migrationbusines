...
func (reader *FileReader) HandleContent(ch2 []model.Excelized, insertable model.Insertable) model.BusinessPageWrapper {
	...
	for _, item := range sections {
		...
		if excl.GR.ReusableType == "reusable-grids" || excl.GR.ReusableType == "reusable-grid-item" {
		...
		if strings.Contains(excl.GR.TypeOfRecord, "hasContact") {
	...
	for _, item := range sections {
		...
		if excl.EN.ReusableType == "reusable-grids" || excl.EN.ReusableType == "reusable-grid-item" {
		...
		if strings.Contains(excl.EN.TypeOfRecord, "hasContact") {
			withContact := hasContact(item, false)
...
func hasContact(items *sll.List, isGr bool) bool {
	item := items.Values()[0]
	itm := item.(model.Excelized)
	return itm.GR.Content == "true" || itm.GR.Content == "TRUE" ||
		itm.GR.Content == "true " || itm.GR.Content == "TRUE " ||
...
func handleReusableGrids(items *sll.List, isGr bool) model.Reusable {
	...
	if isGr {
		for _, item := range items.Values() {
			itm := item.(model.Excelized)
			if len(reusable.Template) == 0 && (itm.GR.TypeOfRecord == "text" || itm.GR.TypeOfRecord == "title" ||
				itm.GR.TypeOfRecord == "Title" ||
				itm.GR.TypeOfRecord == "Title" ||
				itm.GR.TypeOfRecord == "Title " ||
				itm.GR.TypeOfRecord == "Text" ||
				itm.GR.TypeOfRecord == "Text " ||
				itm.GR.TypeOfRecord == "text " || itm.GR.TypeOfRecord == "title " || itm.GR.TypeOfRecord == "Title ") {
				reusable.Title = itm.GR.Content
				reusable.Template = "default"
				reusable.Grids = make([]model.ReusableGridItem, 0)
				continue
			...
			if len(reusable.Template) > 0 && (itm.GR.TypeOfRecord == "Button Name" || itm.GR.TypeOfRecord == "Button Name  ") {
				reusable.Grids[len(reusable.Grids)-1].ButtonText = itm.GR.Content
				continue
			}
			if len(reusable.Template) > 0 && (itm.GR.TypeOfRecord == "Button Url" || itm.GR.TypeOfRecord == "Button Url  ") {
				reusable.Grids[len(reusable.Grids)-1].Url = itm.GR.Content
				continue
			}
			if len(reusable.Template) > 0 && (itm.GR.TypeOfRecord == "img" || itm.GR.TypeOfRecord == "img ") {
				reusable.Grids[len(reusable.Grids)-1].MigratedImageUrl = itm.GR.Content
				continue
			}
		...
		for _, item := range items.Values() {
			itm := item.(model.Excelized)
			if len(reusable.Template) == 0 && (itm.EN.TypeOfRecord == "text" ||
				itm.EN.TypeOfRecord == "Title" ||
				itm.EN.TypeOfRecord == "Title " ||
				itm.EN.TypeOfRecord == "Text" ||
				itm.EN.TypeOfRecord == "Text " ||
				itm.EN.TypeOfRecord == "title" ||
				itm.EN.TypeOfRecord == "text " || itm.EN.TypeOfRecord == "title ") {
				reusable.Title = itm.EN.Content
			...
			if len(reusable.Template) > 0 && (itm.EN.TypeOfRecord == "Button Name" || itm.EN.TypeOfRecord == "Button Name " ||
				itm.EN.TypeOfRecord == "body " || itm.EN.TypeOfRecord == "body ") {
				reusable.Grids[len(reusable.Grids)-1].ButtonText = itm.EN.Content
				continue
			}
			if len(reusable.Template) > 0 && (itm.EN.TypeOfRecord == "Button Url" || itm.EN.TypeOfRecord == "Button Url ") {
				reusable.Grids[len(reusable.Grids)-1].Url = itm.EN.Content
				continue
			}
			if len(reusable.Template) > 0 && (itm.EN.TypeOfRecord == "img" || itm.EN.TypeOfRecord == "img ") {
				reusable.Grids[len(reusable.Grids)-1].MigratedImageUrl = itm.EN.Content
				continue
			}
			if len(reusable.Template) > 0 && (itm.EN.TypeOfRecord == "body" || itm.EN.TypeOfRecord == "body" ||
				itm.EN.TypeOfRecord == "body " || itm.EN.TypeOfRecord == "body ") {
...
func (reader *FileReader) ReadExcelContent(path string) model.Content {
	...
	for index, row := range rows {
		...
		contentEn = append(contentEn, valueStringOtNull(rowsEn[index][4]))
	}
